...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Card className="relative group hover:shadow-lg transition-all duration-300 bg-white border-[#d4c5a0] overflow-hidden">
      {isAdding && (
        <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/60">
          <Loader2 className="h-8 w-8 animate-spin text-[#573e1c]" />
        </div>
      )}
      <div className={`transition duration-200 ${isAdding ? 'blur-sm pointer-events-none select-none' : ''}`}>
